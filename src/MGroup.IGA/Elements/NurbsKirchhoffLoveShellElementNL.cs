...
namespace MGroup.IGA.Elements
{
	...
	public class NurbsKirchhoffLoveShellElementNL : Element, IStructuralIsogeometricElement, ISurfaceLoadedElement
	{
		...
		public Tuple<double[], double[]> CalculateStresses(IElement element, double[] localDisplacements, double[] localdDisplacements)
		{
			...
			var membraneStrain = new double[3];
			var bendingStrain = new double[3];
			...
			for (var j = 0; j < materialsAtThicknessGP.Keys.Count; j++)
			{
				...
				for (var i = 0; i < Bmembrane.GetLength(1); i++)
				{
					...
					{
						membraneStrain[k] += Bmembrane[k, i] * localDisplacements[i];
						bendingStrain[k] += Bbending[k, i] * localDisplacements[i];
					}
				}
		...
		public IMatrix StiffnessMatrix(IElement element)
		{
			...
			for (int j = 0; j < gaussPoints.Length; j++)
			{
				...
				var KmembraneNL = CalculateKmembraneNL(controlPoints, MembraneForces, nurbs, j);

				var KbendingNL = CalculateKbendingNL(controlPoints, BendingMoments, nurbs,
				...
				for (var i = 0; i < stiffnessMatrix.GetLength(0); i++)
				{
					...
					{
						stiffnessMatrix[i, k] += KmembraneNL[i, k];
						stiffnessMatrix[i, k] += KbendingNL[i, k];
					}
				}
			}
